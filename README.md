...
`PhotoImage` image objects, widget visibility and identity-tree queries, plus focused `pack`, `grid`, and `place` layout/info/forget
...
以及聚焦覆盖的 `pack`、`grid`、`place` 布局、信息查询与移除行为、通过 `invoke()` 执行的
