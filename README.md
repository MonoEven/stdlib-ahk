...
`PhotoImage` image objects, image registry queries, widget option-key introspection, visibility, coordinate/size, screen metadata, logical screen and virtual-root dimensions, pixel-distance and RGB color winfo queries, identity-tree queries, and path-to-widget lookup, plus focused `pack`, `grid`, and `place` layout/info/forget/child-query/geometry/row-column-configuration/propagation/anchor
