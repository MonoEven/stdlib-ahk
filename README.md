...
The covered GUI surface currently includes `Tk` roots with option configuration, visibility, state, overrideredirect, iconify/deiconify,
geometry, and window sizing APIs, `Toplevel` windows with state, overrideredirect, iconify/deiconify, geometry, and sizing APIs,
...
当前已覆盖的 GUI 表面包括带选项配置、可见性、状态、overrideredirect、最小化/恢复、几何尺寸与窗口尺寸 API 的 `Tk` 根窗口、带状态、overrideredirect、最小化/恢复、几何尺寸与窗口尺寸 API 的 `Toplevel` 窗口、`Frame`、
...
`Button` / `Checkbutton` / `Radiobutton` command 回调、`Scale` 数值状态、`Scrollbar` 范围状态、`Menu` command 条目、`Entry` 光标与选区状态、焦点管理、事件绑定与合成事件生成、command 参数桥接、Canvas 绘图/图像/窗口项目创建、查询与移动、图像型控件选项，并包含聚焦覆盖的 `after` /
