...
`Button` / `Checkbutton` / `Radiobutton` command 回调、`Scale` 数值状态、`Scrollbar` 范围状态、`Menu` command 条目、`Entry` 光标与选区状态、事件绑定与合成事件生成、command 参数桥接、Canvas 项目查询与移动、图像型控件选项，并包含聚焦覆盖的 `after` /
